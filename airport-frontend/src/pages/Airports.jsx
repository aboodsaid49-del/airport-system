...
export default function Airports() {
  ...
  const handleAdd = async () => {
    if (!form.name || !form.code || !form.city || !form.country) {
      setFormError("Please fill all fields");
      return;
    }
    setSubmitting(true);
    try {
      await axios.post(`${API}/airports`, form);
      showToast("Airport added successfully!");
      setShowModal(false);
      setForm({ name: "", code: "", city: "", country: "" });
      setFormError("");
      fetchAirports();
    } catch (e) {
      setFormError(e.response?.data?.error || "Error adding airport");
    }
    setSubmitting(false);
  };
  ...
  const countries = ["All", ...new Set(airports.map(a => a.country))];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-800 font-sans">
      ...
      <div className="relative z-10 px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-5xl font-black text-white mb-2">🏢 Airports</h2>
            <p className="text-blue-300">Manage all airports worldwide</p>
          </div>
          <button
          ...
          >
            <span className="text-lg">+</span> Add Airport
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            { icon: "🏢", label: "Total Airports", value: airports.length, color: "from-blue-500/20 to-blue-600/10", border: "border-blue-500/30", text: "text-blue-300" },
            { icon: "🌍", label: "Countries", value: new Set(airports.map(a => a.country)).size, color: "from-cyan-500/20 to-cyan-600/10", border: "border-cyan-500/30", text: "text-cyan-300" },
            { icon: "🏙️", label: "Cities", value: new Set(airports.map(a => a.city)).size, color: "from-indigo-500/20 to-indigo-600/10", border: "border-indigo-500/30", text: "text-indigo-300" },
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-3 rounded-xl font-semibold text-sm transition ${viewMode === "grid" ? "bg-blue-500 text-white" : "bg-white/10 text-white/60 border border-white/20"}`}
            >⊞ Grid</button>
            <button
              onClick={() => setViewMode("table")}
              className={`px-4 py-3 rounded-xl font-semibold text-sm transition ${viewMode === "table" ? "bg-blue-500 text-white" : "bg-white/10 text-white/60 border border-white/20"}`}
            >☰ Table</button>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-white/50">
            ...
            <p className="text-lg">Loading airports...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-24 text-white/50">
            <div className="text-6xl mb-4">🏢</div>
            <p className="text-xl font-semibold">No airports found</p>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((airport) => (
              <div key={airport.id} className="group backdrop-blur-md bg-white/10 border border-white/20 rounded-2xl p-6 hover:bg-white/15 transition-all hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/20 hover:border-blue-400/40">
                <div className="flex items-start justify-between mb-4">
                  <div className="text-5xl group-hover:scale-110 transition-transform">{countryFlag(airport.country)}</div>
                  <div className="flex items-center gap-2">
                    <div className="bg-blue-500/30 text-blue-300 border border-blue-500/30 px-3 py-1 rounded-lg text-sm font-mono font-black tracking-widest">
                      {airport.code}
                    </div>
                    <button
                      onClick={() => setShowConfirm(airport.id)}
                ...
                <h3 className="text-white font-bold text-lg leading-tight mb-3">{airport.name}</h3>
                <div className="flex items-center gap-2 text-blue-300 text-sm mb-2">
                  <span>🏙️</span><span className="font-medium">{airport.city}</span>
                </div>
                <div className="flex items-center gap-2 text-blue-200 text-sm">
                  <span>🌍</span><span>{airport.country}</span>
                </div>
                <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between">
                  <span className="text-white/30 text-xs">ID: #{airport.id}</span>
                  <span className="text-blue-400 text-xs font-semibold">✈️ Active</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="backdrop-blur-md bg-white/10 rounded-2xl border border-white/20 overflow-hidden shadow-2xl">
            <table className="w-full text-left">
              ...
              <tbody>
                {filtered.map((airport) => (
                  <tr key={airport.id} className="border-b border-white/10 hover:bg-white/5 transition text-white">
                    <td className="px-6 py-4 text-blue-300 font-mono font-bold">#{airport.id}</td>
                    <td className="px-6 py-4">
                      <span className="bg-blue-500/20 text-blue-300 border border-blue-500/30 px-3 py-1 rounded-lg text-sm font-mono font-black tracking-widest">
                        {airport.code}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <span className="text-2xl">{countryFlag(airport.country)}</span>
                        <span className="font-medium">{airport.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-blue-200">🏙️ {airport.city}</td>
                    <td className="px-6 py-4 text-blue-200">🌍 {airport.country}</td>
                    <td className="px-6 py-4">
                      <button
                        onClick={() => setShowConfirm(airport.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-slate-900 border border-white/20 rounded-2xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-white">🏢 Add Airport</h3>
              <button onClick={() => setShowModal(false)} className="text-white/40 hover:text-white text-xl transition">✕</button>
            </div>
            <div className="space-y-4">
              {[
                { label: "Airport Name", key: "name", placeholder: "e.g. Queen Alia International Airport" },
                { label: "IATA Code", key: "code", placeholder: "e.g. AMM" },
                { label: "City", key: "city", placeholder: "e.g. Amman" },
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-slate-900 border border-red-500/30 rounded-2xl p-8 w-full max-w-sm shadow-2xl text-center">
            ...
            <h3 className="text-xl font-black text-white mb-2">Delete Airport?</h3>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDelete(showConfirm)} className="flex-1 bg-red-500 hover:bg-red-600 text-white py-3 rounded-xl font-bold transition">
                Yes, Delete
              </button>
            </div>
          </div>
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 px-6 py-4 rounded-2xl font-semibold text-white shadow-2xl transition-all ${toast.type === "error" ? "bg-red-500" : "bg-emerald-500"}`}>
